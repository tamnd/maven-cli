...
describes what users get. Pick a format with `-o`, or let mvn choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mvn <command> -o table   # aligned columns for reading
mvn <command> -o jsonl   # one JSON object per line, for piping
mvn <command> -o json    # a single JSON array
mvn <command> -o csv     # spreadsheet friendly
mvn <command> -o tsv     # tab-separated
mvn <command> -o url     # just the URL column
mvn <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mvn <command> --fields id,title,url
```
...
```bash
mvn <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mvn <command>            # a table, because this is a terminal
mvn <command> | wc -l    # JSONL, because this is a pipe
```
